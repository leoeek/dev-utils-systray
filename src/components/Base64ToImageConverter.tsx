...
const Base64ToImageConverter: React.FC = () => {
  ...
  const handleShowImage = useCallback(() => {
    const currentBase64 = textAreaRef.current?.value || '';

    if (!currentBase64.trim()) {
      setError('Por favor, insira uma string Base64.');
      return;
    }

    if (!currentBase64.startsWith('data:image/')) {
    ...
    if (newWindow) {
      newWindow.document.write(`
        <html>
          ...
          <body>
            <img src="${currentBase64}" alt="Imagem convertida de Base64" />
          </body>
        </html>
      `);
